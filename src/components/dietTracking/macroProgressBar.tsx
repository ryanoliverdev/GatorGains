    ...
    return (
      <div className="flex gap-2 ">
        ...
        <Progress
          ...
          value={Math.min(
            (amount * 4 * 100) /
              ((macroInfo.dailyProteinPercentage / 100) * dailyCals),
            100
          )}
        />
        <h1 className="text-sm w-[150px] text-right">
          {(((amount * 4) / dailyCals) * 100).toFixed(1)}% /{' '}
          {macroInfo.dailyProteinPercentage}%
        </h1>
      </div>
    );
  } else if (type === 'Carbs') {
    return (
      <div className="flex gap-2 ">
        ...
        <Progress
          ...
          value={Math.min(
            (amount * 4 * 100) /
              ((macroInfo.dailyCarbsPercentage / 100) * dailyCals),
            100
          )}
        />
        <h1 className="text-sm w-[150px] text-right">
          {(((amount * 4) / dailyCals) * 100).toFixed(1)}% /{' '}
          {macroInfo.dailyCarbsPercentage}%
        </h1>
      </div>
    );
  } else if (type === 'Fat') {
    return (
      <div className="flex gap-2">
        ...
        <Progress
          ...
          value={Math.min(
            (amount * 9 * 100) /
              ((macroInfo.dailyFatPercentage / 100) * dailyCals),
            100
          )}
        />
        <h1 className="text-sm w-[150px] text-right">
          {(((amount * 9) / dailyCals) * 100).toFixed(1)}% /{' '}
          {macroInfo.dailyFatPercentage}%
        </h1>
      </div>
    );
  }
